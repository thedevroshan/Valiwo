...
export const PublicPosts = async (
  ...
  try {
    ...
    const publicPosts: IPost[] = await Promise.all(
...
export const PaidPosts = async (req: Request, res: Response): Promise<void> => {
  try {
    const rawPaidPosts: IPost[] = await Post.find({
      user: req.signedInUser?.id,
      is_paid: true,
    });

    const paidPosts: IPost[] = await Promise.all(
      rawPaidPosts.map(async (post): Promise<IPost> => {
        if (!post.is_paid) {
          const likeCount = await Like.countDocuments({
            post_user: post.users,
          });
...
export const DeletePost = async (
  ...
  try {
    ...
    const isFileDeleted = await storage.deleteFile(
      process.env.POST_BUCKET_ID as string,
      post.post_file_id[0] as string
    );
...
export const EditPost = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const isEdited = await Post.findOneAndUpdate(
      { _id: post_id, users: req.signedInUser?.id },
      {
        $set: {
          caption,
        },
